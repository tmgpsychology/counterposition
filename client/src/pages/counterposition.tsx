...
export default function Counterposition() {
  ...
  useEffect(() => {
    if (step === "result" && score && !savedRef.current) {
      savedRef.current = true;
      if (user && !shared) {
        apiRequest("POST", "/api/exercises/counterposition", {
          belief,
          counterArgument,
          grade: score.grade,
      ...
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          {step !== "belief" && (
            <button
              onClick={handleReset}
              className="px-4 py-2 text-sm uppercase tracking-widest border-2 border-foreground rounded-md hover:bg-foreground hover:text-background transition-all"
              ...
              <div className="space-y-6">
                ...
                <AnimatePresence>
                  {showSignupPrompt && !user && (
                    <GuestSignupPrompt onDismiss={() => setShowSignupPrompt(false)} />
                  )}
                </AnimatePresence>
              </div>
            </div>
